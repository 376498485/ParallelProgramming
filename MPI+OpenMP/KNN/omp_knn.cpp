...
void knn(float **X_train, int *y_train, float **X_test, int *y_test, int k){
    ...
    for(int times=0; times<100; times++){ // 100 times, to calculate time
    ...
    printf("Serial time:%fs\n",(double)(clock()-start_cpu)/CLOCKS_PER_SEC/100);
    printf("accuracy = %.2f%%\n", 100*accuracy(predict, y_test, len_test));
}
void omp_knn(float **X_train, int *y_train, float **X_test, int *y_test, int k){
    int len_train = (int)(N-N*TEST_SIZE);  // the length of train set
	int len_test = (int)(N*TEST_SIZE);     // the length of test set
    int predict[len_test];
...
int main(){
    ...
    file_read();
    train_test_split(X_train, y_train, X_test, y_test, features, labels, TEST_SIZE, RANDOM_STATE);
    knn(X_train, y_train, X_test, y_test, K);
    omp_knn(X_train, y_train, X_test, y_test, K);
    return 0;
}
