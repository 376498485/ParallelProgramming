...
int main(int argc, char* argv[])
{
    ...
    int size = 1024; //size must be 32 64 128 256...
        ...
        for (int i = 0; i < size; ++i)
            for (int j = 0; j < size; ++j)
                if(C[i][j] != A[i][j])
                    correctness = false;
        if(correctness)
            printf("MPI - Correct: matrix size = %d, number of processes = %d,  time = %lf sec \n",  size, num_procs, elapsed_secs );

        else
            printf("MPI - Incorrect: matrix size = %d, number of processes = %d,  time = %lf sec \n",  size, num_procs, elapsed_secs );
